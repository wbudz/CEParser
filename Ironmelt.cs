...
namespace CEParser
{
    public class Ironmelt
    {
        ...
        DecodePhase phase = DecodePhase.RecordingString;
        int strlen = 0;

        Stack<DataType> typeInheritance = new Stack<DataType>();
        DataType type = DataType.Unspecified;
        Stack<bool?> quotedInheritance = new Stack<bool?>();
        bool quoted = false;
        int enforcedOffset = 0;

        #region Settings

        public bool EnforceDateDatatype { get; set; }
        public bool IncludeHour { get; set; }
        ...
        public Ironmelt(string gameToken, string[] tokens, Encoding encoding)
        {
            game = gameToken;
            strlen = gameToken.Length + 3;
            this.tokens = new BinaryTokens(tokens);
            this.encoding = encoding;
        }

        public static string[] ReadTokensFile(string path)
        {
            // Read tokens file
            var output = new List<string>();
            using (var r = new StreamReader(System.IO.File.Open(path, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)))
            {
                while (!r.EndOfStream)
                {
                    string line = r.ReadLine();
                    if (line.Length <= 14 || r.BaseStream.Position == 0) continue; // empty code
                    output.Add(line);
                }
            }
                    ...
                    else
                    {
                        ...
                        else if (enforcedOffset > 0)
                        {
                            ...
                            if (enforcedOffset == 0)
                            {
                                ...
                                Result = new DecodeResult(output, quoted, unknown, type);
                            }
                            return;
                        }
        ...
        string GetHexString(byte b1, byte b2)
        {
        ...
        private void InterpretVariable(string text, Stack<Node> hierarchy)
        {
            ...
            if (game.StartsWith("hoi4"))
            {
                switch (text)
                {
                    ...
                    case "session":
                        ...
                        break;
                    case "action":
                        if (IsParent(hierarchy, 1, "proposed_diplo_action"))
                        {
                            type = DataType.Integer;
                            quoted = false;
                        }
                        ...
                        break;
                    case "ironman":
                        if (IsParent(hierarchy, 1, "gameplaysettings"))
                        {
                            type = DataType.Integer;
                            quoted = false;
                        }
            ...
            if (game.StartsWith("eu4"))
            {
                switch (text)
                {
                    ...
                    case "target":
                        if (IsParent(hierarchy, 1, "focal_point") || IsParent(hierarchy, 1, "estate_influence"))
                        {
                            type = DataType.Integer;
                            quoted = false;
                        }
                    ...
                    case "claim":
                        if (IsParent(hierarchy, 1, "monarch") || IsParent(hierarchy, 1, "heir"))
                        {
                            type = DataType.Integer;
                            quoted = false;
                        }
                        ...
                        break;
                    case "estate":
                        if (Int32.TryParse(GetParent(hierarchy, 1), out _))
                        {
                            type = DataType.Integer;
                            quoted = false;
                        }
                        ...
                        break;
                    case "capital":
                        if (GetParent(hierarchy, 1).Length == 3 && GetParent(hierarchy, 1)[0] != '-')
                        {
                            type = DataType.Integer;
                            quoted = false;
                        }
                    ...
                    case "cavalry":
                    case "artillery":
                        if (IsParent(hierarchy, 2, "army_templates"))
                        {
                            quoted = false;
                            type = DataType.Integer;
                        }
                ...
                switch (text)
                {
                    case "type":
                            ...
                            IsParent(hierarchy, 2, "character"))
                        {
                            ...
                            IsParent(hierarchy, 2, "offmap_powers"))
                        {
                            type = DataType.String;
                            quoted = true;
                        }
        ...
        bool IsParent(Stack<Node> hierarchy, int levelsUp, string text)
        {
        ...
        string GetParent(Stack<Node> hierarchy, int i)
        {
            try
            {
                return hierarchy.ElementAt(i).Name;
            }
            catch
            {
                return "";
        ...
        static Dictionary<int, string> intLookup = new Dictionary<int, string>();
        static Dictionary<int, string> floatLookup = new Dictionary<int, string>();
        static Dictionary<int, string> float5Lookup = new Dictionary<int, string>();
        static Dictionary<int, string> dateLookup = new Dictionary<int, string>();
        int val;

        string GetValue(ref int strlen)
        {
            string output;
            switch (type)
            {
                ...
                case DataType.Date:
                    ...
                    {
                        output = DecodeDate(val, IncludeHour);
        ...
        private static string DecodeDate(int input, bool includeHour)
        {
    ...
    public struct DecodeResult
    {
        ...
        public DecodeResult(string token, bool quoted, string unknown, DataType type)
        {
            Token = token;
            Quoted = quoted;
            Unknown = unknown;

            if (type == DataType.Boolean || type == DataType.Float || type == DataType.Float5 || type == DataType.Integer)
            {
                Quoted = false;
            }
        }
